...
namespace Coderr.Server.PostgreSQL.Core.Applications
{
    [ContainerService]
            ...
            using (var cmd = (DbCommand) _uow.CreateCommand())
            {
                cmd.CommandText =
                    @"INSERT INTO Applications (Name, AppKey, CreatedById, CreatedAtUtc, ApplicationType, SharedSecret, EstimatedNumberOfErrors, NumberOfFtes) 
                        VALUES(@Name, @AppKey, @CreatedById, @CreatedAtUtc, @ApplicationType, @SharedSecret, @EstimatedNumberOfErrors, @NumberOfFtes);RETURNING Id;";
