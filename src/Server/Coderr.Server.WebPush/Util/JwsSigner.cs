...
namespace Coderr.Server.WebPush.Util
{
    internal class JwsSigner
    {
        ...
        public string GenerateSignature(Dictionary<string, object> header, Dictionary<string, object> payload)
        {
            ...
            var signature = UrlBase64Helper.Encode(a.Concat(b).ToArray());
        ...
        {
            var encodeHeader = UrlBase64Helper.Encode(Encoding.UTF8.GetBytes(JsonConvert.SerializeObject(header)));
            var encodePayload = UrlBase64Helper.Encode(Encoding.UTF8.GetBytes(JsonConvert.SerializeObject(payload)));
