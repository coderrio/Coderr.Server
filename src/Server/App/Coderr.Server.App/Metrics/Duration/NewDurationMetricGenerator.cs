...
{
    [Metric(Name)]
    public class NewDurationMetricGenerator : IMetricGenerator
    {
        public const string Name = "NewDuration";

        private readonly IAdoNetUnitOfWork _unitOfWork;

        public NewDurationMetricGenerator(IAdoNetUnitOfWork unitOfWork)
        {
        ...
        {
            DisplayName = "Time to action", Description = "Number of days until someone starts to work on an new error. Unassigned errors are excluded from this indicator"
        };
        ...
        public async Task Generate(IMetricGenerationContext context)
        {
            ...
            var sql = $@"select ApplicationId, {sqlDateFormat} AS Date, AVG(DATEDIFF(day, CreatedAtUtc, AssignedAtUtc)) Duration
