...
export class HomeComponent implements OnInit, OnDestroy {
  ...
  async selectReport(reportId: number): Promise<void> {
    ...
    if (report.contextCollections.length > 0) {
      ...
      // Resume with the one that we used previously.
      ...
      collection = report.contextCollections.filter(x => x.name === "HighlightCollections" || x.name === "HighlightCollection");
      if (collection.length > 0) {
        this.selectCollection(collection[0].name);
        return;
      }

      // When the user supplies data, show it.
      collection = report.contextCollections.filter(x => x.name === 'ContextData');
      if (collection.length > 0) {
        this.selectCollection(collection[0].name);
        return;
      }
